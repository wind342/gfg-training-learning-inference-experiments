...
def _protected_tree_vector_sha256(actual: dict[str, str]) -> str:
    material = json.dumps(
        actual,
        ensure_ascii=False,
        separators=(",", ":"),
...
def protected_path_audit() -> dict[str, Any]:
    ...
    return {
        "schema_version": "protected-path-audit-v2.1",
        "audit_subject": {
            "revision": "CONTAINING_COMMIT",
            "binding": "PROTECTED_PATH_TREE_OBJECT_IDS",
            "protected_tree_vector_sha256": (
                _protected_tree_vector_sha256(actual)
            ),
            "verification_ref": "HEAD",
        },
        "comparisons": comparisons,
        "protected_status_entries": (
            status.splitlines() if status else []
        ),
        "gates": gates,
