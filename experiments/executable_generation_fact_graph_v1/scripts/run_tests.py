...
ARTIFACT_PATH = (
    Path(__file__).resolve().parents[1] / "artifacts" / "test_results.json"
)
...
def main() -> int:
    runs = [
        _run(
            "v1_focused",
            ["-q", "experiments/executable_generation_fact_graph_v1/tests"],
        ),
        _run("core", ["-q", "tests/core"]),
        _run("full_repository", ["-q"]),
    ]
    result = {
        "schema_version": "executable-generation-fact-graph-tests-v1",
        "status": (
            "PASS"
