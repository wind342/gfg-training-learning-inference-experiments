...
def _matching_rules(
    node: Any, outcome_kind: str, contract: dict[str, Any]
) -> list[dict[str, Any]]:
    transform = node.realized_transformation
    occurrence_type = node.concrete_occurrence["occurrence_type"]
    return [
        rule
        for rule in contract["rules"]
        if rule["transform_operation"] == transform.get("operation")
        and rule["occurrence_type"] == occurrence_type
        and rule["relation_role"] == node.relation_role
...
def project_graph_to_signed_algebra(
    ...
    for node in sorted(
        ...
        if (
            ...
            raise ValueError("SIGNED_PROJECTION_EFFECT_IDENTITY_INVALID")
        source = node.source_reference["entity"]
        fact = {
            ...
            "omega": {
                ...
                "occurrence_payload": node.concrete_occurrence[
                    "occurrence_payload"
                ],
                "occurrence_type": node.concrete_occurrence[
                    "occurrence_type"
                ],
                "stable_instance_key": node.concrete_occurrence[
                    "stable_instance_key"
                ],
            },
        }
        ...
        if sign == "positive":
            terms.append(
                SignedPair.positive_variable(effect_identity, multiplicity)
            )
        elif sign == "negative":
            terms.append(
                SignedPair.negative_variable(effect_identity, multiplicity)
            )
        ...
        contributions.append(
            {
                ...
                "effect_identity": effect_identity,
                "multiplicity": multiplicity,
                "occurrence_identity": node.concrete_occurrence[
                    "stable_instance_key"
                ],
    ...
    return {
        ...
        "snapshot_id": validated_graph.graph.metadata.source_snapshot_ids[0],
